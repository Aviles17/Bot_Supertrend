...
class TestPosicions(unittest.TestCase):
    ...
    def setUp(self):
        # XRPUSDT fue seleccionado para la prueba debido a su bajo costo
    ...
    def test_make_close_order(self):
        if self.stock_df["Polaridad"].iloc[1] == 1:
            order = Posicion('Buy', 'XRPUSDT', self.qty_xrp, self.stock_df["Polaridad"].iloc[1], str(round(float(self.stock_df['Supertrend'].iloc[0]), 4)), float(self.stock_df['Close'].iloc[0]), str(self.stock_df['Time'].iloc[0]), float(
                self.stock_df['Open'].iloc[0]), float(self.stock_df['High'].iloc[0]), float(self.stock_df['Low'].iloc[0]), float(self.stock_df['Volume'].iloc[0]), float(self.stock_df['DEMA800'].iloc[0]))
        elif self.stock_df["Polaridad"].iloc[1] == -1:
            order = Posicion('Sell', 'XRPUSDT', self.qty_xrp, self.stock_df["Polaridad"].iloc[1], str(round(float(self.stock_df['Supertrend'].iloc[0]), 4)), float(self.stock_df['Close'].iloc[0]), str(self.stock_df['Time'].iloc[0]), float(
                self.stock_df['Open'].iloc[0]), float(self.stock_df['High'].iloc[0]), float(self.stock_df['Low'].iloc[0]), float(self.stock_df['Volume'].iloc[0]), float(self.stock_df['DEMA800'].iloc[0]))

        res = order.make_order(self.client)
        self.assertEqual(res['retMsg'], 'OK')
        time.sleep(15)  # Espera 15 segundos para cerrar la orden
        res = order.close_order(self.client, float(
            self.stock_df['Close'].iloc[0]))
        self.assertEqual(res['retMsg'], 'OK')
    ...
    def test_modificar_stoploss(self):
        if self.stock_df["Polaridad"].iloc[2] == 1:
            order = Posicion('Buy', 'XRPUSDT', self.qty_xrp, self.stock_df["Polaridad"].iloc[1], str(round(float(self.stock_df['Supertrend'].iloc[0]), 4)), float(self.stock_df['Close'].iloc[0]), str(self.stock_df['Time'].iloc[0]), float(
                self.stock_df['Open'].iloc[0]), float(self.stock_df['High'].iloc[0]), float(self.stock_df['Low'].iloc[0]), float(self.stock_df['Volume'].iloc[0]), float(self.stock_df['DEMA800'].iloc[0]))
            order.make_order(self.client)
            # Mover 10% del stoploss
            new_stoploss = float(order.stoploss) + (float(order.stoploss)*0.1)
            time.sleep(15)  # Espera 15 segundos para vender la mitad
        elif self.stock_df["Polaridad"].iloc[2] == -1:
            order = Posicion('Sell', 'XRPUSDT', self.qty_xrp, self.stock_df["Polaridad"].iloc[1], str(round(float(self.stock_df['Supertrend'].iloc[0]), 4)), float(self.stock_df['Close'].iloc[0]), str(self.stock_df['Time'].iloc[0]), float(
                self.stock_df['Open'].iloc[0]), float(self.stock_df['High'].iloc[0]), float(self.stock_df['Low'].iloc[0]), float(self.stock_df['Volume'].iloc[0]), float(self.stock_df['DEMA800'].iloc[0]))
        ...
        res = order.modificar_stoploss(
            self.client, str(round(new_stoploss, 4)))
        self.assertEqual(res['retMsg'], 'OK')
        time.sleep(15)
        res = order.close_order(self.client, float(
            self.stock_df['Close'].iloc[0]))
